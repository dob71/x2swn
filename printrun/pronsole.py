...
tempreading_exp = re.compile("(^T:| T:|ok T\d?:)")
...
class pronsole(cmd.Cmd):
    ...
    def postcmd(self, stop, line):
        ...
        if self.p.online and self.dynamic_temp:
            self.p.send_now(REPORT_TEMP_CMD)
        self.prompt = self.promptf()
        return stop
    ...
    def precmd(self, line):
        if line.upper().startswith("M114"):
            self.userm114 += 1
        elif line.upper().startswith(REPORT_TEMP_CMD):
            self.userm105 += 1
        return line
...
(this will terminate the print)."))
        ...
        if(not os.path.exists(os.path.join(x2swProfilesPath, '.use_local'))):
            ...
            try:
                if(not os.path.exists(x2swProfilesPath)):
                    self.log(_("Creating x2sw profiles path: " + x2swProfilesPath))
                    os.makedirs(x2swProfilesPath)
                if((not os.path.exists(rcPathName)) and os.path.exists(rcDistroFilename)):
                    self.log(_("Deploying pronsole distro rc file to: " + rcPathName))
                    shutil.copyfile(rcDistroFilename, rcPathName)
            except:
                self.log(_("Failure!!!"))
        else:
            rcPathName = rcDistroFilename
        self.log(_("Loading default pronsole rc file: " + rcPathName))
        try:
            self.load_rc(rcPathName)
        except IOError:
            self.log(_("Cannot load: " + rc_filename))
    ...
    def do_slice(self, l):
        ...
        try:
            if settings:
                command = self.expandcommand(self.settings.sliceoptscommand)
                self.log(_("Entering slicer settings: %s") % command)
                run_command(command, blocking = True)
            else:
                command = self.expandcommand(self.settings.slicecommand)
                stl_name = l[0]
