...
class printcore():
            ...
            if line.lower().startswith("resend") or line.startswith("rs"):
                ...
                while len(linewords) != 0:
                    try:
                        ...
                        logging.info("Resending from: %s" % toresend)
                        break
                    except:
                        pass
                self.clear = True
    ...
    def _sendnext(self):
        ...
        if not (self.printing and self.printer and self.online):
            self.clear=True
            return
        if self.resendfrom < self.lineno and self.resendfrom > -1 and self.resendfrom >= (self.lineno - self.senttokeep):
        ...
        if calcchecksum and not self.printer_tcp:
            ...
            if "M110" not in command:
                ...
                if lineno > self.senttokeep:
                    del self.sentlines[lineno - self.senttokeep - 1]
        if self.printer:
